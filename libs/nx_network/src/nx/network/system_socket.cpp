...
{
    qDebug() << "KHOI TAO SOCKET O DAY type : " << type << " protocol : " << protocol;
#ifdef _WIN32
    ...
    std::chrono::milliseconds timeout)
{
    ...
    if (resolvedEntries.empty())
        return false;
    qDebug() << "CommunicatingSocket<SocketInterfaceToImplement>::connect 2";
    ...
    while (!resolvedAddresses.empty())
    {
        ...
        if (connectToIp(SocketAddress(std::move(ip), remoteAddress.port), timeout))
            return true;
        qDebug() << "CommunicatingSocket<SocketInterfaceToImplement>::connect 4";
    }
    qDebug() << "CommunicatingSocket<SocketInterfaceToImplement>::connect 5";
...
bool UDPSocket::joinGroup(const HostAddress& multicastGroup, const HostAddress& multicastIF)
{
    ...
    qDebug() << "IP MULTI CAST: " << multicastGroup.toString().c_str();
    qDebug() << "IP LOCAL: " << multicastIF.toString().c_str();
    if (setsockopt(
            handle(), IPPROTO_IP, IP_ADD_MEMBERSHIP,
            (raw_type *)&multicastRequest, sizeof(multicastRequest)) < 0)
    {
    ...
    SocketAddress* const sourceAddress)
{
    ...
    //qDebug() << nx::format("IP2 VA PORT2: %1:%2 ").args(m_prevDatagramAddress.address, m_prevDatagramAddress.port);
