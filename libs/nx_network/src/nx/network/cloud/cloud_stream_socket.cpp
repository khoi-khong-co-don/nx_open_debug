...
{
    qDebug() << "KHOI CloudStreamSocket::getLocalAddress()";
    if (m_socketDelegate)
...
{
    qDebug() << "KHOI CloudStreamSocket::recv";
    NX_ASSERT(!SocketGlobals::aioService().isInAnyAioThread());

    if (!m_socketDelegate)
    {
        qDebug() << "KHOI khong co socket Delegate";
        SystemError::setLastErrorCode(SystemError::notConnected);
        return -1;
    }
   //  qDebug() << "KHOI CloudStreamSocket::recv" << m_socketDelegate->recv(buffer, bufferLen, flags);
