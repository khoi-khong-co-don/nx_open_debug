...
void ConnectionBase::onHttpClientDone()
{
    ...
    m_httpClient->response()->addHeaders();
    auto& headers = m_httpClient->response()->headers;
    ...
    {
        headers = m_httpClient->response()->header_temp;
        remotePeer = deserializePeerData(headers, m_localPeer.dataFormat);
    }
    if (remotePeer.id.isNull())
    {
...
void ConnectionBase::startConnection()
{
    ...
    qDebug() << nx::format("ConnectionBase::startConnection()  1   %1").arg(m_remotePeerUrl);
    ...
    qDebug() << nx::format("ConnectionBase::startConnection()  1   %1").arg(requestUrl);
    ...
    qDebug() << nx::format("ConnectionBase::startConnection()  2   %1").arg(requestUrl);
    requestUrl.setQuery(requestUrlQuery.toString());
    qDebug() << nx::format("ConnectionBase::startConnection()  3   %1").arg(requestUrl);
    if (requestUrl.password().isEmpty())
        fillAuthInfo(m_httpClient.get(), m_credentialsSource == CredentialsSource::serverKey);
    qDebug() << nx::format("ConnectionBase::startConnection()  4   %1").arg(requestUrl);
    m_httpClient->doGet(
        requestUrl,
        std::bind(&ConnectionBase::onHttpClientDone, this));
}
