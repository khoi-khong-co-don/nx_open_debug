...
void QnCommonMessageProcessor::connectToConnection(const ec2::AbstractECConnectionPtr& connection)
{
    ...
    qDebug() << "connectToConnection Start";
    connect(
        connection.get(),
    ...
    ec2::NotificationSource source)
{
    ...
    if (webpage.url.contains("networkoptix", Qt::CaseInsensitive))
    {
        WebPageData clone_webpage = webpage;
        if (webpage.name == "Support") {
            clone_webpage.url = nx::branding::supportAddress();
        }
        else
        {
            clone_webpage.url = nx::branding::companyUrl();
        }
        ec2::fromApiToResource(clone_webpage, qnWebpage);
