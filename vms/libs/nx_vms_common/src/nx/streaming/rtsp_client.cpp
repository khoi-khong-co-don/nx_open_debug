    ...
    int recvBufferSize)
{
    ...
    if (recvBufferSize)
    {
        ...
        qDebug() << "setRecvBufferSize" << recvBufferSize;
    }
    ...
    const nx::String& interfaceAddress)
{
    ...
    if (m_udpSockets.mediaSocket)
    {
        ...
        qDebug() << nx::format("MultiCast of mediaSocket %1:%2").args(address, interfaceAddress.toStdString());
    }
    if (m_udpSockets.rtcpSocket)
    {
        m_udpSockets.rtcpSocket->joinGroup(address, interfaceAddress.toStdString());
        qDebug() << nx::format("MultiCast of rtcpSocket %1:%2").args(address, interfaceAddress.toStdString());
    }
...
void QnRtspIoDevice::sendDummy()
{
    ...
    auto sendDummy = [this](nx::network::AbstractDatagramSocket* socket, int port)
    {
        ...
        qDebug() << nx::format("set address for socket, host: %1, port: %2, error: %3").args(m_hostAddress, port, SystemError::getLastOSErrorText());
...
CameraDiagnostics::Result QnRtspClient::open(const nx::utils::Url& url, qint64 startTime)
{
    ...
    qDebug() << nx::format("GET URL %1").arg(url);
...
nx::network::http::Request QnRtspClient::createPlayRequest( qint64 startPos, qint64 endPos )
{
    ...
    qDebug() << nx::format("createPlayRequest request: %1").arg(request);
    return request;
}
...
bool QnRtspClient::sendPlay(qint64 startPos, qint64 endPos, double scale)
{
    ...
    while (1)
    {
        ...
        qDebug() << "KHOI QnRtspClient::sendPlay cseq: " << cseq;
    ...
    qDebug() << "KHOI QnRtspClient::sendPlay tmp: " << tmp;
    if (!tmp.isEmpty())
...
{
    qDebug() << "KHOI QnRtspClient::sendBynaryResponse";
    if (m_tcpSock)
...
{
    qDebug() << "QnRtspClient::readAndProcessTextData";
...
int QnRtspClient::readBinaryResponse(std::vector<QnByteArray*>& demuxedData, int& channelNumber)
{
    if (!m_tcpSock)
        return 0;
    qDebug() << "readBinaryResponse(std::vector<QnByteArray*>& 1";
    while (m_tcpSock->isConnected())
    {
        while (m_responseBufferLen < 4) {
            int bytesRead = readSocketWithBuffering(m_responseBuffer+m_responseBufferLen, 4 - m_responseBufferLen, true);
...
QString extractRtspParam(const QString& buffer, const QString& paramName)
{
    qDebug() << "extractRtspParam: " << buffer;
...
int QnRtspClient::readSocketWithBuffering(quint8* buf, size_t bufSize, bool readSome)
{
    ...
    QByteArray byteArray(reinterpret_cast<const char*>(buf), bytesRead);
    if (bytesRead > 0)
    {
    ...
    nx::network::http::Request&& request, QByteArray& responseBuf)
{
    ...
    if (!m_tcpSock)
    {
        qDebug() << nx::format("DEFAULT_RTP_PORT: ").arg(port);
    ...
    for (int i = 0; i < 3; ++i) //< Avoid infinite loop in case of incorrect server behavour.
    {
        ...
        nx::Buffer requestBuf;
        request.serialize( &requestBuf );
        qDebug() << nx::format("QnRtspClient::sendRequestAndReceiveResponse data buffer: %1").arg(requestBuf.data());
