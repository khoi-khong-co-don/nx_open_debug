...
void DeprecatedMulticastFinder::updateInterfaces()
{
    ...
    for (const QString& addressStr: nx::network::getLocalIpV4AddressList())
    {
        ...
        try
        {
            ...
            qDebug() << nx::format("DEPRECATED SOCKET LOCAL: %1").args(sock->getLocalAddress());
            ...
            if (m_serverSocket)
            {
                if (!m_serverSocket->joinGroup(m_multicastGroupAddress, address))
                {
                    ...
                    qDebug() << nx::format("Unable to join multicast group %1 on interface %2: %3").args(m_multicastGroupAddress, address);
                }
                else
                {
            ...
            if (!m_pollSet.add(sockPtr, aio::etRead, sockPtr))
                ...
                qDebug() << nx::format("PollSet(%1s): Added %2 socket").args(m_pollSet.size(), address);
        }
