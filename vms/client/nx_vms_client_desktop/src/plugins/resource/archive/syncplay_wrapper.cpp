...
static const std::chrono::milliseconds kCacheTimeout {1};
...
public:
    ...
    mutable nx::Mutex timeMutex;
    mutable nx::Mutex readerListMutex;
    ...
    mutable nx::utils::BasicElapsedTimer<std::chrono::microseconds> gotCacheTimer;
...
qint64 QnArchiveSyncPlayWrapper::getDisplayedTime() const
{
    ...
    if (d->lastJumpTime == DATETIME_NOW && !d->paused)
        return DATETIME_NOW;
    else if (d->enabled && d->bufferingCnt > 0 && d->bufferingTime != AV_NOPTS_VALUE)
...
void QnArchiveSyncPlayWrapper::erase(QnAbstractArchiveDelegate* value)
{
    ...
    reinitTimeIfBufferingIsFinishedUnsafe();
}
...
void QnArchiveSyncPlayWrapper::onBufferingStarted(QnlTimeSource* source, qint64 bufferingTime)
{
    ...
    for (QList<ReaderInfo>::iterator i = d->readers.begin(); i < d->readers.end(); ++i)
    {
        qDebug() << nx::format("KHOI ReaderInfo %1").arg(i->cam);
        qDebug() << "KHOI onBufferingStarted ";
        if (i->cam == source)
        {
            if (!i->buffering)
                d->bufferingCnt++;
...
void QnArchiveSyncPlayWrapper::reinitTimeIfBufferingIsFinishedUnsafe()
{
    Q_D(QnArchiveSyncPlayWrapper);
...
qint64 QnArchiveSyncPlayWrapper::getCurrentTime() const
{
    ...
    if (!d->gotCacheTimer.hasExpired(kCacheTimeout))
        return d->cachedTime;

    d->gotCacheTimer.restart();
...
double QnArchiveSyncPlayWrapper::getSpeed() const
{
    Q_D(const QnArchiveSyncPlayWrapper);
    return d->speed;
}
