    ...
    QnWorkbenchContextAware(parent)
{
    ...
    qDebug() << __func__ << "Tao model start";
    createSystemModel();
    qDebug() << __func__ << "Tao model end";
}
...
void WelcomeScreen::openConnectingTile(std::optional<RemoteConnectionErrorCode> errorCode)
{
    ...
    qDebug() << nx::format("errorCode: %1").arg(errorCode);
    ...
    bool storePassword)
{
    ...
    if (!system)
        return;
    qDebug() << nx::format("Url : %1  serverId %2   userName %3  password %4  ").args(serverUrl, serverId, userName, storePassword);
    qDebug() << "connect server___________________________ 2";
    ...
    nx::utils::Url url = parseConnectionUrlFromUserInput(serverUrl);
    qDebug() << nx::format("Url : %1  serverId %2   userName %3  password %4  ").args(url, serverId, localSystemId, storePassword);
    connectToSystemInternal(
        systemId,
        serverId.isEmpty() ? std::nullopt : std::optional{QnUuid::fromStringSafe(serverId)},
...
static SystemsVisibilitySortFilterModel* createVisibilityModel(QObject* parent)
{
    ...
    qDebug() << __func__ << " setVisibilityScopeFilterCallbacks START";
    ...
    qDebug() << __func__ << " setVisibilityScopeFilterCallbacks END";
    qDebug() << __func__ << " setForgottenCheckCallback START";
    ...
    qDebug() << __func__ << " setForgottenCheckCallback END";
...
void WelcomeScreen::createSystemModel()
{
    ...
    qDebug() << __func__ << "vao de tao model start";
    m_systemModel = new ConnectTilesProxyModel(this);
    qDebug() << __func__ << "Tao moi ham connect proxy model xong";
    qDebug() << __func__ << "setCloudVisibilityScopeCallbacks start";
    m_systemModel->setCloudVisibilityScopeCallbacks(                // 0 chay thang nay
        []()
        {
    ...
    qDebug() << __func__ << "setCloudVisibilityScopeCallbacks end";
    ...
    const auto connectFunction =
        [this, systemId, logonData, completionTracker]()
        {
            ...
            NX_DEBUG(this, "Connecting to the system %1 after click on tile",
                logonData.address);
            qDebug() << nx::format("Connecting to the system %1 after click on tile").arg(logonData.address);
