    ...
    base_type(parent)
{
    ...
    SearchAddressManager* searchUrlManager = new SearchAddressManager(this);            //tim duoc server da login
    ...
    addSystemsFinder(recentLocalSystemsFinder, kRecentFinder);                          // them server da login vao model
...
void QnSystemsFinder::addSystemsFinder(QnAbstractSystemsFinder* finder, int priority)
{
    ...
    *connections << connect(finder, &QObject::destroyed, this,
    ...
    for (const auto& system: finder->systems())
    {
        qDebug() << __func__ << " no thay server da login cho nay";
        onBaseSystemDiscovered(system, priority);
    }
}
